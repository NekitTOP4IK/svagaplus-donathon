...
class StyleGeneratorService {
  ...
  static String generateCSS(TimerStyle style) {
    ...
    buffer.writeln(
      '  padding: ${style.paddingTop.toInt()}px ${style.paddingRight.toInt()}px ${style.paddingBottom.toInt()}px ${style.paddingLeft.toInt()}px;',
    );
    buffer.writeln('  box-sizing: border-box;');
    buffer.writeln(
      '  margin: ${style.marginTop.toInt()}px ${style.marginRight.toInt()}px ${style.marginBottom.toInt()}px ${style.marginLeft.toInt()}px;',
    );

    // Background
    if (style.showBackground) {
      buffer.writeln(
        '  background-color: ${colorToHex(style.backgroundColor)};',
      );
    } else {
      buffer.writeln('  background-color: transparent;');
    }
    ...
    if (style.showBorder) {
      buffer.writeln(
        '  border: ${style.borderWidth.toInt()}px solid ${colorToHex(style.borderColor)};',
      );
    ...
    if (style.showTextShadow) {
      buffer.writeln(
        '  text-shadow: ${style.textShadowOffsetX.toInt()}px ${style.textShadowOffsetY.toInt()}px ${style.textShadowBlur.toInt()}px ${colorToHex(style.textShadowColor)};',
      );
    }
    ...
    if (style.animationType != 'none') {
      buffer.writeln(
        '  animation: ${style.animationType} ${style.animationDuration.toStringAsFixed(1)}s ${style.animationTimingFunction} infinite;',
      );
    }
    ...
    if (style.separatorAnimation != 'none') {
      buffer.writeln(
        '  animation: ${style.separatorAnimation} ${style.separatorAnimationDuration.toStringAsFixed(1)}s ${style.animationTimingFunction} infinite;',
      );
    }
    buffer.writeln('}');
  ...
  static String generateHTMLPreview(
    TimerStyle style, {
