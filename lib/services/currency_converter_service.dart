...
class CurrencyConverterService {
  static final CurrencyConverterService _instance =
    ...
    try {
      ...
      if (source == 'er-api') {
        ...
        if (response.statusCode == 200) {
          ...
          LogManager.info(
            'Конвертер валют обновлен (ER-API). База: USD, RUB: ${_rates['RUB']}',
          );
        } else {
        ...
        if (response.statusCode == 200) {
          ...
          _lastUpdate = DateTime.now();
          LogManager.info(
            'Конвертер валют обновлен (Frankfurter). База: $base, RUB: ${_rates['RUB']}',
          );
        } else {
        ...
        final url = Uri.parse(
          'https://ratata.money/api/v1/rates/latest?base=RUB',
        );
        final request = await client.getUrl(url);
        final response = await request.close();
        ...
        if (response.statusCode == 200) {
          ...
          _lastUpdate = DateTime.now();
          LogManager.info(
            'Конвертер валют обновлен (Ratata). База: RUB, USD: ${_rates['USD']}',
          );
        } else {
        ...
        if (response.statusCode == 200) {
          ...
          _lastUpdate = DateTime.now();
          LogManager.info(
            'Конвертер валют обновлен (CBR-JSON). База: RUB, USD: ${_rates['USD']}',
          );
        } else {
        ...
        if (response.statusCode == 200) {
          ...
          final regex = RegExp(
            r'<CharCode>([A-Z]+)<\/CharCode>.*?<Nominal>(\d+)<\/Nominal>.*?<Value>([\d,]+)<\/Value>',
          );
          ...
          _lastUpdate = DateTime.now();
          LogManager.info(
            'Конвертер валют обновлен (CBR-XML). База: RUB, USD: ${_rates['USD']}',
          );
        } else {
