...
class DonationAlertsAdapter extends BaseDonationServiceAdapter {
  ...
  String get _wsUrl =>
      'wss://$_socketServer.donationalerts.ru/socket.io/?EIO=3&transport=websocket';
  ...
  void _handleMessage(dynamic message) {
    ...
    if (data == '40' || data.startsWith('40{')) {
      ...
      LogManager.info(
        'DonationAlerts: namespace подключён, отправка токена...',
      );
  ...
  void _scheduleReconnect() {
    if (_reconnectAttempts >= _maxReconnectAttempts) {
      ...
      LogManager.error(
        'DonationAlerts: превышено число попыток переподключения',
      );
      updateStatus(ConnectionStatus.error);
      return;
    }
    ...
    _reconnectTimer = Timer(_reconnectDelay * (_reconnectAttempts + 1), () {
      ...
      LogManager.info(
        'DonationAlerts: попытка переподключения #$_reconnectAttempts',
      );
      updateStatus(ConnectionStatus.reconnecting);
      _initWebSocket();
    });
