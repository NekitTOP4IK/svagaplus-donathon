...
### Установка артефактов (сырых версий / альфа версий если они есть)
...
В настройках можно подключить SVAGA+ через сопряжение с браузером. Интеграция
по умолчанию выключена, а время за новую подписку и продление по умолчанию
составляет 15 минут. Device token хранится только в защищённом хранилище
Windows и не записывается в `data.json`.

При включённой интеграции события, полученные во время временного отсутствия
сети, догружаются после переподключения. При выключенной интеграции события за
этот период намеренно пропускаются. История SVAGA+ хранится локально и работает
офлайн: для записи можно нажать «Отменить» с подтверждением или «Вернуть».
Эти действия меняют только текущий таймер и не изменяют серверную историю.

Для сборки с сервером задайте URL во время сборки через
`--dart-define=SVAGAPLUS_BASE_URL=...`; подробности приведены в
[`docs/SVAGAPLUS_BUILD.md`](docs/SVAGAPLUS_BUILD.md).
